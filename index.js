...
async function handleInvestment(text, waNum, person) {
  ...
  if (t.match(/^(invest|investment|portfolio)\s*(list|show|status)?\s*$/)) {
    ...
    // Fetch live EUR→IDR rate, fallback 17500
    let eurToIdr = 17500;
    try {
      const rateRes = await fetch('https://open.er-api.com/v6/latest/EUR');
      if (rateRes.ok) {
        const rateData = await rateRes.json();
        eurToIdr = rateData?.rates?.IDR || 17500;
      }
    } catch (_) {}

    // Build per-item lines + running total (EUR items converted to Rp)
    let totalRp = 0;
    const lines = inv.map(i => {
      if (i.amountEur) {
        const asRp = i.amountEur * eurToIdr;
        totalRp += asRp;
        return `• ${i.type}: €${i.amountEur.toLocaleString('en')} (≈ Rp${Math.round(asRp).toLocaleString('id')})`;
      } else {
        totalRp += (i.amountRp || 0);
        return `• ${i.type}: Rp${Math.round(i.amountRp || 0).toLocaleString('id')}`;
      }
    }).join('\n');

    const totalEur = Math.round(totalRp / eurToIdr).toLocaleString('en');
    return `📊 *Portfolio as of ${date}*\n\n${lines}\n\n*Total: Rp${Math.round(totalRp).toLocaleString('id')} (≈ €${totalEur})*\n_Rate: 1 EUR = Rp${Math.round(eurToIdr).toLocaleString('id')}_`;
  }
